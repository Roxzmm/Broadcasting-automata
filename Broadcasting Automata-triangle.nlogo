...
to setup
  ...
  ;; setting for number of spoints and radius
  set num-spoints 2
  set radius2 [ 3.1 6.1 ]
...
;  set chaincodes replace-item 1 chaincodes ["0" "1" "2" "3"]
  ...
  let numofrow (ceiling (width / (sqrt 3) * 2 / 3) - 1)
  create-ordered-cells ((ceiling (height / ( 3 / 2))) * numofrow)
  ask cells [
    let row (floor (who / numofrow))
    let inx (who mod numofrow)

    if row mod 2 = 0
    [
      if inx mod 2 = 0
      [ setxy (3 * (sqrt 3) * (inx / 2)) (row * ( 3 / 2))
         set shape "triangle30" ]
      if inx mod 2 = 1
      [ setxy (3 * (sqrt 3) * ((inx - 1) / 2) + (sqrt 3)) (row * ( 3 / 2))
         set shape "triangle-30" ]
    ]
    if row mod 2 = 1
    [
      if inx mod 2 = 0
      [ setxy (3 / 2 * (sqrt 3) * (inx + 1)) (row * ( 3 / 2))
         set shape "triangle30" ]
      if inx mod 2 = 1
      [ setxy (3 * (sqrt 3) * ((inx - 1) / 2) + 5 / 2 * (sqrt 3)) (row * ( 3 / 2))
         set shape "triangle-30" ]
    ]
...
;    set shape "triangle"
  ...
  ask dots [
    ...
    let tempneighbors []
    ask dots with [ distance myself <= (sqrt 3 + 0.1) and distance myself > 0 ] [
...
false
0
Polygon -7500403 true true 270 150 90 45 90 255

triangle30
false
0
Polygon -7500403 true true 30 150 210 255 210 45
