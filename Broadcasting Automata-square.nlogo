...
dots-own [values amplitudes t1-amplitudes t2-amplitudes neighbor-dots msg-radius isobstacle?]
...
;  set chaincodes replace-item 1 chaincodes ["0" "1" "2" "3"]
  ...
  ask dots [
    ...
    set neighbor-dots turtles-on neighbors4
...
;    set msg-radius [ 1 ]
  ...
  add-spoint-manually 100 150
  add-spoint-manually 200 150
end
...
;      if (position self t0-activepoints != false) [
;        set t0-activepoints remove-item (position self t0-activepoints) t0-activepoints
;      ]
...
;      set cache (lput selected cache)
;      set cache remove-duplicates cache]
;;    set spoints (lput temp spoints)
;;    display]
;  ]
;end
...
to add-spoint
  ...
  [
    ...
    if selected != nobody [
      ask selected [
;        set t1-amplitudes replace-item 0 t1-amplitudes -10
        set color red
        set size 0.5]
...
;    set t0-activepoints (lput item 0 cachespoints t0-activepoints)
;    let templist item tempindex t0-activepoints
;    show templist
;    set templist replace-item 0 templist (item 0 cachespoints)
;    show templist
;    set t0-activepoints replace-item tempindex t0-activepoints (item 0 cachespoints)
;    show templist
...
;      let templist item tempindex t0-activepoints
;      set templist (lput item 0 cachespoints templist)
;      set templist replace-item 0 (item tempindex t0-activepoints) templist
...
;    if position "3" values != false and position "3" values != false [ set tempcolor yellow ]
    ...
    set color tempcolor
    ask patch-here [ set pcolor tempcolor ]]
end
...
;    set (item index t1-activepoints) remove-duplicates (item index t1-activepoints)
...
;    set templist replace-item index t1-activepoints templist

  ]

;  let step radii
;;  let step 1
;  let targetdistance (iterations * step)
;  let targetdistanceoff ((iterations - 1) * step)
;;  if targetdistance <= radii [
;    ask dots with [ distance centerpoint <= targetdistance and distance centerpoint > targetdistanceoff] [
;      let tempindex position centerpoint spoints
;      assign-message tempindex
;;      set values replace-item tempindex values send-message tempindex
;      paint self ]
end

;; apply chain codes (chaincodes must be stored as list)
;; all startpoints must start at the same time
;; ? how to assign message to the starting point itself
...
;  let tempinx (iterations mod num) - 1
;  if iterations mod num = 0 [
;    set tempinx num - 1]
;  let codes item index chaincodes
;  let code item tempinx codes
...
;  show result
;  set t2-amplitudes replace-item 0 t2-amplitudes (item 0 t1-amplitudes)
;  set t2-amplitudes replace-item 0 t2-amplitudes result
  set amplitudes replace-item 0 amplitudes result
;  if (item 0 amplitudes != 0) [
...
;      set (item index t1-activepoints) (lput self (item index t1-activepoints))
;      set t1-activepoints replace-item index t0-activepoints (item 0 cachespoints)

;      set t1-activepoints (lput self t1-activepoints)
